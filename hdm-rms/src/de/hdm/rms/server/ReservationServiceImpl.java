...
public class ReservationServiceImpl extends RemoteServiceServlet implements
	...
	@Override
	public ArrayList<ReservationListObj> loadAllReservationsAsList(int hostUserId) {
		System.out.println( hostUserId  + " -- HOst User ID --");
		ArrayList<ReservationListObj> reservationlist = resMapper.loadAllReservationsAsList(hostUserId);
		if (!reservationlist.isEmpty()) {
			return reservationlist;
		} else {
			return null;
...
//				} else 
	...
	@Override
	 public void sendInvitation(ArrayList<UserRms> userInvitationArray, Reservation res, String roomName, int userId) {
	  UserRms userOwn =  uMapper.OneUserById(userId);
		System.out.println(" user hat id   : "+ userOwn.getId());

	  
	  for(int i=0; i < userInvitationArray.size()  ; i++){
	   String getEmailAdress = uMapper.getEmailByAdress(userInvitationArray.get(i).getId());
		System.out.println("impl user klau : " + getEmailAdress);

	   UserRms userClient  = userInvitationArray.get(i);
		System.out.println("der elch wars:"+ getEmailAdress.toString());

	   sendMail(userClient,userOwn, res,roomName ,userInvitationArray  );
		System.out.println("impl mail send haiaiai : ");

	  }
	 }
	
	public void sendMail( UserRms uTo , UserRms userOwn, Reservation res, String roomName, ArrayList<UserRms> userInvitationArray) {
		  String userAgent = getThreadLocalRequest().getHeader("User-Agent");

		  Reservation r = res; 
		  
		  String nickname = uTo.getNickName(); 
		  String hostname = uTo.getNickName(); 

		  String Name = uTo.getFirstName() + " " + uTo.getLastName(); 
		  String eventname = res.getTopic(); 
		...
		//  String subject= "Einladung zu Veranstaltung: " + eventname ;
		  ...
		  String text_message =  "Hallo: " + nickname  
		  + ", \n\n" + 
		  "du wurdest von: " + hostname 
		  +"\n"+
		  "eigenladen an der Veranstaltung " + eventname
		  + " \n" + 
		  "am " + startTime + "um " + endTime + "im Raum: " + raum + "teilzunehmen."
	...
	@Override
	public Invitation setInviteStatus(int invStatus, int resId, int memId) {
			Boolean status =null;
			
			status = iMapper.updateInvitation(invStatus, resId, memId);
		
		return null;
	}

	@Override
	public int loadAcceptionStatus(int resId, int memId) {

		int acceptionStatus = iMapper.selectInvStatus(resId, memId);
		
//		if(acceptionStatus != 0){
			
		System.out.println("acceptionStatus return Impl: "+acceptionStatus);
