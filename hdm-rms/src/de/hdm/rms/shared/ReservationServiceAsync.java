...
//	void login(String hostPageBaseURL, AsyncCallback<LoginInfo> asyncCallback);
	...
	void setInviteStatus(int invStatus, int resDd, int userId, AsyncCallback<Invitation> asyncCallback);

	void loadAcceptionStatus(int resId, int memId, AsyncCallback<Integer> callback);

 
	void loadAllReservationsAsList(int temp_user_id,
			AsyncCallback<ArrayList<ReservationListObj>> asyncCallback);

	void loadAllInvitationData(int temp_user_id,
			AsyncCallback<ArrayList<InvitationListObj>> asyncCallback);
